...
class Instagram
{
    ...
    public static function withCredentials($username, $password, $sessionFolder = null)
    {
        ...
        if (is_null($sessionFolder)) {
            ...
            static::$cookieDir = $sessionFolder . "cookie" . DIRECTORY_SEPARATOR;
            $cookiePath = static::$cookieDir . $username . ".json";
        } else {
            static::$cookieDir = $sessionFolder . DIRECTORY_SEPARATOR . "cookie" . DIRECTORY_SEPARATOR;
            $cookiePath = static::$cookieDir . $username . ".json";
        }

//        var_dump($cookiePath);
//        var_dump(file_exists($cookiePath) ? "true" : "false");
//        sleep(10000);
//        die;
        $cookieParsed = [];
        if (is_string($sessionFolder)) {
            if (file_exists($cookiePath)) {
                $cookieParsed = json_decode(file_get_contents($cookiePath), true);
            }
...
//        var_dump(static::$instanceCache );
//        die;
        $instance = new self();
        $instance->sessionUsername = $username;
        $instance->sessionPassword = $password;
        $instance->userSession = $cookieParsed;
        $instance->cookiePath = $cookiePath;
        return $instance;
    }
...
//        return md5(implode(':', [$this->getRhxGis(), $variables]));
    ...
    public function story($owner, $reel, $timestamp)
    {
        $body = [
            'reelMediaId' => $reel,
            'reelMediaOwnerId' => $owner,
            'reelId' => $owner,
            'reelMediaTakenAt' => $timestamp,
            'viewSeenAt' => $timestamp
        ];
        $response = Request::post(Endpoints::getSeenUrl(), $this->generateHeaders($this->userSession), $body);
        if ($response->code !== static::HTTP_OK) {
            throw new InstagramException('Response code is ' . $response->code . '. Body: ' . static::getErrorBody($response->body) . ' Something went wrong. Please report issue.', $response->code);
        }
...
//        echo $response->raw_body.PHP_EOL;
    }
...
//            die(json_encode($jsonResponse['data']['reels_media']));
            $UserStories = UserStories::create();
            $UserStories->setJson($jsonResponse['data']['reels_media']);
...
//        die;
        ...
        if ($force || !$this->isLoggedIn($session)) {
            ...
            if ($response->code !== static::HTTP_OK) {
                ...
                } else if (isset($response->body->two_factor_required) && $response->body->two_factor_required === true) {
//                    $this->challangeId = $response->body->two_factor_info->two_factor_identifier;
//                    echo PHP_EOL . $this->challangeId . PHP_EOL;
                    $this->saveTFA($response->headers, $cookies, $response->body);
                    die("Enter verification code");
...
//        var_dump($this->userSession);
     ...
     *
     */
    public function saveTFA($header, $cookies, $body)
    {
        $tfa = static::$root . DIRECTORY_SEPARATOR . "2fa" . DIRECTORY_SEPARATOR . $this->sessionUsername;
        if (!file_exists($tfa)) {
            mkdir($tfa);
        }
        file_put_contents($tfa . DIRECTORY_SEPARATOR . "body.json", json_encode($body));
        file_put_contents($tfa . DIRECTORY_SEPARATOR . "header.json", json_encode($header));
        file_put_contents($tfa . DIRECTORY_SEPARATOR . "cookies.json", json_encode($cookies));
    }
    ...
    public function twoStep($code)
    {
        ...
        $cachedString = static::$instanceCache->getItem($this->sessionUsername);
        $body = json_decode(file_get_contents($tfa . DIRECTORY_SEPARATOR . "body.json"));
        $headers = json_decode(file_get_contents($tfa . DIRECTORY_SEPARATOR . "header.json"),true);
        $cookies = json_decode(file_get_contents($tfa . DIRECTORY_SEPARATOR . "cookies.json"),true);

        $new_cookies = $this->parseCookies($headers);
        $cookies = array_merge($cookies, $new_cookies);
        $cookie_string = '';
        foreach ($cookies as $name => $value) {
            $cookie_string .= $name . '=' . $value . '; ';
        ...
        $url = Endpoints::TAF_URL;
        $post_data = [
            'username' => $this->sessionUsername,
            'verificationCode' => $code,
            'identifier' => $body->two_factor_info->two_factor_identifier
        ];
    ...
    public function saveCookie()
    {
        if (!file_exists($this->cookiePath)) {
            if (!file_exists(static::$cookieDir)) {
                mkdir(static::$cookieDir);
            }
            $fp = fopen($this->cookiePath, "wb");
            fclose($fp);
        }
        file_put_contents($this->cookiePath, json_encode($this->userSession));
    }
